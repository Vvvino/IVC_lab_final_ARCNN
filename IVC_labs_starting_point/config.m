...
ParameterStruct(1).name='sail.tif - starting algorithm';                % Name of the investigated compression mode
ParameterStruct(1).input_image_filename='data/images/sail.tif';         % Input image file name
...
ParameterStruct(1).output_stream_filename='data/compressed/sail.ivc';   % Output file name (Compressed)
...
ParameterStruct(1).reconstructed_image_filename='data/reconstructed/sail.tif';    % Output file name (Reconstructed)
...
ParameterStruct(3).name='smandril.tif - starting algorithm';                % Name of the investigated compression mode
ParameterStruct(3).input_image_filename='data/images/smandril.tif';         % Input image file name
...
ParameterStruct(3).output_stream_filename='data/compressed/smandril.ivc';   % Output file name (Compressed)
...
ParameterStruct(3).reconstructed_image_filename='data/reconstructed/smandril.tif';    % Output file name (Reconstructed)

%--------------------------
% write configuration file
%--------------------------
image1 = double(imread(ParameterStruct(1).input_image_filename));
image2 = double(imread(ParameterStruct(2).input_image_filename));
image3 = double(imread(ParameterStruct(3).input_image_filename));

PMF1= stats_marg(image1);
PMF2= stats_marg(image2);
PMF3= stats_marg(image3);
marg_entropy1 = calc_entropy(image1);
marg_entropy2 = calc_entropy(image2);
marg_entropy3 = calc_entropy(image3);
