...
path(path,'decoder')            
path(path,'analysis')
Frame1 = double(imread('foreman20_40_RGB/foreman0020.bmp'));
Frame2 = double(imread('foreman20_40_RGB/foreman0021.bmp'));
lena = double(imread('data/images/lena.tif'));
[Frame1_YCbCr_PCA,T1,offset1] = RGB2YCbCr(Frame1);
[Frame2_YCbCr_PCA,T2,offset2] = RGB2YCbCr(Frame2);
[lena_YCbCr_PCA,T_lena,offset_lena] = RGB2YCbCr(lena);
Frame1_YCbCr_ict = ictRGB2YCbCr(Frame1);
Frame2_YCbCr_ict = ictRGB2YCbCr(Frame2);
%% use lena small to train huffman table
f = waitbar(0,'Please wait...');
g=1;
qScale = 1;
% for qScale = 0.15:0.3:2
k_small  = IntraEncode(lena_YCbCr_PCA, qScale); 
k        = IntraEncode(Frame1_YCbCr_PCA, qScale);

PMF_1 = hist(k_small,min(k):max(k));
PMF_1 = PMF_1/sum(PMF_1);
[BinaryTree_1, HuffCode_1, BinCode_1, Codelengths_1] = buildHuffman(PMF_1);
%% use first frame with lena small huffman table to transfer coding and calculate PSNR and bit/pixel
bytestream_1 = enc_huffman_new(k-min(k)+1, BinCode_1, Codelengths_1);
% bitPerPixel(1) = (numel(bytestream_1)*8) / (numel(Frame1)/3);
bitPerPixel(1) = 8;
k_rec = dec_huffman_new(bytestream_1, BinaryTree_1, length(k)) + min(k)-1;
I_rec = (IntraDecode((k_rec), size(Frame1),qScale));
Frame1_RGB = YCbCr2RGB(Frame1_YCbCr_PCA,T1,offset1);

% PSNR(1) = calcPSNR(Frame1_RGB, I_rec_RGB_1);
PSNR(1) = calcPSNR(Frame1,Frame1_RGB);
% Frame_new_rec{1} = I_rec;
Frame_new_rec{1} = RGB2YCbCr(Frame1_RGB);
...
mv_indices = SSD(Frame1_YCbCr_ict, Frame2_YCbCr_ict);%Mv between Frame1 and 2
...
for i = 1:20
    ...
    [Frame_pre_YCbCr_PCA,T_pre,offset_pre] = RGB2YCbCr(double(imread(Frame_pre)));
    [Frame_new_YCbCr_PCA,T_new,offset_new] = RGB2YCbCr(double(imread(Frame_new)));
        
    motion_vectors = SSD(Frame_new_rec{i},Frame_new_YCbCr_PCA);%mv between 2 Frames
    ...
    Prediction_error{i} = (Frame_new_YCbCr_PCA - Frame_sender_rec);% error between sender reconstrected Frame and true Frame
    
    Frame_new_rec{i+1} = Frame_receive_rec + Prediction_error{i};% receiver reconstruted new Frame with decoded error
    PSNR(i+1) = calcPSNR(YCbCr2RGB(Frame_new_YCbCr_PCA,T_new,offset_new),YCbCr2RGB(Frame_new_rec{i+1},T_new,offset_new));% calculate the PSNR between receiver reconstruted Frame and true Frame
    
end

% calculate minimum of all residuals
for h =1:20
        err_en{h} = Prediction_error{i}(:)';
...
Frame2_rec = SSD_rec(Frame1_YCbCr_PCA, mv_indices);%reconstructed Frame2 with mv and Frame1 YCbCr
Prediction_error_1 = (Frame2_rec - Frame2_YCbCr_PCA);%get the error of reconstructed Frame2 and true Frame2 YCbCr
Prediction_error_1 = Prediction_error_1(:)';
PMF_3 = hist(Prediction_error_1,mini_err_en_value:maxi_err_en_value);%get the pmf of transfer encoding error
